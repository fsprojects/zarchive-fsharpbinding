...
module Common = 
  ...
  let generateReferences (items:ProjectItemCollection) configSelector shouldWrap = seq { 
    ...
    if not coreRef then
      ...
      | Some fn -> yield "-r:/usr/lib/mono/4.0/FSharp.Core.dll"
  ...
  let fscPath = 
    ...
    | _ -> if Environment.runningOnMono then "/usr/lib/mono/4.0/fsc.exe" else "fsc.exe"
  ...
  let fsiPath = 
    ...
    | Some(dir) when ScriptOptions.safeExists(Path.Combine(dir, "fsi.exe")) ->  
        ...
        // Fallback case - depends on the platform
    | _ -> if Environment.runningOnMono then "/usr/lib/mono/4.0/fsi.exe" else "fsi.exe"
