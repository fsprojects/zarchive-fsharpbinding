...
type ParameterDataProvider(nameStart: int) = 
    inherit MonoDevelop.Ide.CodeCompletion.ParameterDataProvider(nameStart)
    //interface IParameterDataProvider with 

    let mutable meths: MethodOverloads = new MethodOverloads()

    member x.Meths
        //with get() = meths 
        with set v = meths <- v

    override x.get_Count() = meths.Methods.Length
    ...
    try
     if (completionChar <> '(' && completionChar <> ',' && completionChar <> ')' ) then null else
      ...
      | Some meths -> 
        ...
        let pdp = new ParameterDataProvider (startOffset) 
        pdp.Meths <- meths
        pdp :> _ 
    with _ -> null
